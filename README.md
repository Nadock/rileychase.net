...
- [ ] Rename `live` command to `dev`
- [ ] Add CLI command to scaffold new pages
- [ ] Refactor render pipeline in a single per-file pipeline that splits into other sub-pipelines
  - Initially; `markdown` (default), `blog_index`, `blog_post`, and `static` types will have their own sub-pipelines
  - Each type has it's own default template name `${type}.html` (? maybe)
- [ ] Better name for `site_generator` CLI
- [ ] Pyprojects script support
